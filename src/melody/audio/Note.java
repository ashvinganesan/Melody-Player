...
/**
 * Each Note object represents a musical note or rest. A Note encapsulates a
 * pitch (A-G), a duration in seconds, an octave, an accidental (sharp, flat, or
 * natural), and a flag of whether it is the start/end of a repeated section or
 * not. A song or melody can be thought of as a list or array of Note objects.
 */
public class Note {
     ...
     * @param pitch Note's pitch from Pitch.A through Pitch.G, or Pitch.R for a
     * rest.
     * @param octave Note's octave from OCTAVE_MIN through OCTAVE_MAX inclusive.
     * @param accidental Note's accidental from Accidental.SHARP, FLAT, or
     * NATURAL.
    ...
    /**
     * Returns true if o refers to a Note object with the same state as this
     * Note object; otherwise false.
     *
    ...
    /**
     * Returns this Note's accidental value of SHARP, FLAT, or NATURAL. The
     * accidental value is meaningless for a rest; this method will return
     * Accidental.NATURAL by default if called on a rest.
     */
    ...
    /**
     * Returns this Note's octave. The octave value is meaningless for a rest;
     * this method will return OCTAVE_MIN + 1 by default if called on a rest.
     */
    ...
    /**
     * Returns true if this Note is a rest. Equivalent to checking whether this
     * note's pitch is Pitch.R. Provided for convenience.
     */
    ...
    /**
     * Plays this note through the underlying audio system. Also prints a
     * message to the system console for debugging. If the audio system is muted
     * or paused, the note may not play.
     */
    public void play() {
        if (DEBUG) {
    ...
    /**
     * Sets this Note's accidental value to be the given value: SHARP, FLAT, or
     * NATURAL. The accidental value is meaningless for a rest, but the Note
     * object still maintains an accidental value internally (initially
     * Accidental.NATURAL) which is ignored.
     *
     * @param accidental Note's accidental from Accidental.SHARP, FLAT, or
     * NATURAL.
    ...
    /**
     * Sets this Note's octave to be the given value. The octave value is
     * meaningless for a rest, but the Note object still maintains an octave
     * value internally (initially OCTAVE_MIN + 1) which is ignored.
     *
     * @param octave Note's octave from OCTAVE_MIN through OCTAVE_MAX inclusive.
     * @throws IllegalArgumentException if octave is not between OCTAVE_MIN and
